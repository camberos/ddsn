...
req.open("GET.html",url,async);if(!async){req.send(undefined);if(req.readyState!=4){log.error("Request failed: "+req.readyState);return undefined;}
if(!getXML)
...
return str;}
function getRootWindow(){var win=window;while(win!=undefined){try{if(win===win.parent){break;}else if(win.parent!=undefined&&win.parent.document.location.href.indexOf("selenium-server/index.html")!=-1){break;}
...
protocol="index.html";var host=document.location.host;if(host==undefined)
...
pos=domain.indexOf("index.html");if(pos!=-1){domain=domain.substring(0,pos);}
...
function getPhotoURL(url){var loc=document.location;if(/\w+:\/\/.+/.test(url)){return url;}else if(url.indexOf("index.html")===0){return loc.protocol+"//"+loc.host+url;}else{var path=loc.pathname;var pos=path.lastIndexOf("index.html");if(pos!=-1){path=path.substring(0,pos);}
